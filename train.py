...
test0_data = batchify(corpus.data['valid0'], eval_batch_size, shuffle=False)
...
for epoch in range(1, args.epochs+1):
    ...
    while niter < len(train1_data) and niter < len(train0_data):
        ...
        for i in range(args.niters_ae):
            ...
            total_loss_ae2, _ = \
                train_ae(2, train0_data[niter], total_loss_ae2, start_time, niter)
            
            # train classifier ----------------------------
            classify_loss1, classify_acc1 = train_classifier(1, train1_data[niter])
            classify_loss2, classify_acc2 = train_classifier(2, train0_data[niter])
            ...
            classifier_regularize(2, train0_data[niter])
        ...
        for k in range(niter_gan):
            ...
            for i in range(args.niters_gan_d):
                ...
                if i % 2 == 0:
                    ...
                    batch = train0_data[random.randint(0, len(train0_data)-1)]
                    whichdecoder = 2
    ...
    test_loss, accuracy = evaluate_autoencoder(2, test0_data[:1000], epoch)
    print('-' * 89)
    print('| end of epoch {:3d} | time: {:5.2f}s | test loss {:5.2f} | '
          'test ppl {:5.2f} | acc {:3.3f}'.
          format(epoch, (time.time() - epoch_start_time),
...
test_loss, accuracy = evaluate_autoencoder(2, test0_data, epoch+1)
print('-' * 89)
print('| end of epoch {:3d} | time: {:5.2f}s | test loss {:5.2f} | '
      'test ppl {:5.2f} | acc {:3.3f}'.
      format(epoch, (time.time() - epoch_start_time),
